...
import { isDisabledCollection, isExcludedCategory, isExcludedCollection, isFavoritedCollection, toggleExcludedCollection, toggleFavoriteCollection } from '../store'
...
<template>
  <div>
    ...
    >
      <template v-if="!isDisabledCollection(c.id)">
        <RouterLink
          :to="`/collection/${c.id}`"
          flex-auto
